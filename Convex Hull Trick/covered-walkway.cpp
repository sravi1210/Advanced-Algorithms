...
int main(){
	...
	for(ll i=0;i<N;i++){
		dp.push_back(ch.query(X[i]) + C + (X[i]*X[i]));
		if(i == 0){
			dp[i] -= (X[i]*X[i]);
		}
		else{
		    dp[i] = min(dp[i-1]+C, dp[i]); //important condition in this problem taking care of the condition that certain block can also be taken alone instead of relying on other.
		}
		
		ll slope = (-2)*X[i];
		ll intercept = ((i==0)?0:dp[i-1]) + (X[i]*X[i]);
  
		ch.add(slope, intercept);
	}
	cout<<dp[N-1]<<endl;
	return 0;
}